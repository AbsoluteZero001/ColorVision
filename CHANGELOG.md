...
- 新增“日志”主导航页面，按北京时间显示上传原图、识别区域图和 RGB/LAB/HEX/ROI 参数。
- 新增本地 SQLite `upload_logs` 表，首次启动自动初始化且不写入种子数据。
- 上传日志图片保存在本机 `data/logs/images/`，支持单条删除、全部清空和图片预览。
- 新增日志记录、日志图片、留存天数和最大条数配置。
